package sk.sandeep.databindingexample.util
...
import sk.sandeep.databindingexample.R
...
fun popularityIcon(view: ImageView, popularity: ProfileViewModel.Popularity) {
...
fun tintPopularity(view: ProgressBar, popularity: ProfileViewModel.Popularity) {
...
private fun getAssociatedColor(popularity: ProfileViewModel.Popularity, context: Context): Int {
    return when (popularity) {
        ProfileViewModel.Popularity.NORMAL -> context.theme.obtainStyledAttributes(
            intArrayOf(android.R.attr.colorForeground)
        ).getColor(0, 0x000000)
        ProfileViewModel.Popularity.POPULAR -> ContextCompat.getColor(context, R.color.popular)
        ProfileViewModel.Popularity.STAR -> ContextCompat.getColor(context, R.color.star)
    }
}

private fun getDrawablePopularity(
    popularity: ProfileViewModel.Popularity,
    context: Context
): Drawable? {
    return when (popularity) {
        ProfileViewModel.Popularity.NORMAL -> {
            ContextCompat.getDrawable(context, R.drawable.ic_person_black_96dp)
        }
        ProfileViewModel.Popularity.POPULAR -> {
            ContextCompat.getDrawable(context, R.drawable.ic_whatshot_black_96dp)
        }
        ProfileViewModel.Popularity.STAR -> {
            ContextCompat.getDrawable(context, R.drawable.ic_whatshot_black_96dp)
        }
    }
}
